package w3_resources.part5_arr2;

import java.util.Arrays;

public class Arr34 { 
    // https://www.w3resource.com/java-exercises/array/java-array-exercise-34.php
    // sort
    // brute force all subset using set (as on the page)
    // Find length of longest consecutive sequence
    int solve(int[] arr) {
        return -1;
    }
    int two_pointer (int[] arr) {
        int max = arr[0];
        for (int i = 1; i < arr.length; i++)
            max = Math.max(max,arr[i]);
        int[] table = new int[max+1];
        for (int i = 0; i < arr.length; i++) {
            table[arr[i]] = 1; // mark presence
        }

        int longest = 0;
        int len = 0;
        int left = 0, right = 0;

        while (right < table.length) {
            if (table[right] == 1) {
                len++;
                longest = Math.max(longest, len);
                right++;
            } else {
                len = 0;          // reset streak
                left = ++right;   // move both pointers to next start
            }
        }
        return longest;
    }
    int one_pointer(int[] arr) {
        int max = arr[0];
        for (int i = 1; i < arr.length; i++)
            max = Math.max(max, arr[i]);

        int[] table = new int[max + 1];
        for (int i = 0; i < arr.length; i++) {
            table[arr[i]] = 1; // mark presence
        }

        int longest = 0;
        int len = 0;

        for (int i = 0; i < table.length; i++) {
            if (table[i] == 1) {
                len++;
                longest = Math.max(longest, len);
            } else {
                len = 0;
            }
        }
        return longest;
    }
    int min_max(int[] arr) {
        int max = arr[0], min = arr[0];
        for (int val : arr) {
            max = Math.max(max, val);
            min = Math.min(min, val);
        }

        // Table to mark existence of numbers
        boolean[] exists = new boolean[max + 1];
        for (int val : arr) {
            exists[val] = true;
        }

        int longest = 0;
        int current = 0;

        for (int i = min; i <= max; i++) {
            if (exists[i]) {
                current++;
                longest = Math.max(longest, current);
            } else {
                current = 0;
            }
        }
        return longest;
    }

    int longest_streak(int[] arr) {
        int max = arr[0];
        for (int i = 1; i < arr.length; i++) 
            max = Math.max(max, arr[i]);
        
        int[]len = new int[max+1];
        for (int i = 0; i < arr.length; i++) {
            len[arr[i]] = 1;
        }
        int streak = len[0];
        int idx = 1;
        int longest = 0;
        while (idx < len.length) {
            if (len[idx] == 1) {
                streak++;
                longest = Math.max(longest, streak);
            } else {
                streak = 0;
            }
            idx++;
        }
        return longest;
    }    
    public static void main(String[] args) {
        int[] arr = {49, 1, 3, 200, 2, 4, 70, 5};
        Arr34 sol = new Arr34();
        System.out.println( sol.two_pointer(arr));
    }    
}
